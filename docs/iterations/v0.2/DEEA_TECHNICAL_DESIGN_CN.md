...
**状态**：对应已合并至 `main` 的 [代码 PR #4](https://github.com/gwx4399-cell/coca-word-frequency-tool/pull/4)；本文档在后续文档 PR 中提交｜2026-09-23。  
...
未引入新存储键或迁移，已有 `deea.essays.v1` 记录可直接读取；浏览器清除本地数据会丢失作文。没有作文原文的联网请求，也没有埋点或 AI 模型调用。GitHub Pages 工作流在 `main` 推送时触发；本文件不声称已核实公开页面的部署状态。
