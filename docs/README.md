...
| v0.4：原句证据与词形高亮 | [PR #9](https://github.com/gwx4399-cell/coca-word-frequency-tool/pull/9) 的分支已实现，待评审与合并；不宣称公开部署 | [PRD 中文](./iterations/v0.4/DEEA_PRD_CN.md) · [PRD English](./iterations/v0.4/DEEA_PRD_EN.md) · [原句证据设计图](./iterations/v0.4/DEEA_EVIDENCE_DESIGN.svg) | [技术设计中文](./iterations/v0.4/DEEA_TECHNICAL_DESIGN_CN.md) · [Technical design English](./iterations/v0.4/DEEA_TECHNICAL_DESIGN_EN.md) · 37 个测试、类型检查、构建通过。 |

**首页文档更新**：[PR #8](https://github.com/gwx4399-cell/coca-word-frequency-tool/pull/8) 已合并；将[根目录 README](../README.md)改为当前产品与作品集总览，并校正本页及 v0.3 文档的合并状态。该 PR 不改变产品行为或测试规则，因此不另立功能版本。
...
按[文档与发布约定](./WORKFLOW.md)为每次用户可见的功能或规则更新保留中英 PRD、中英技术设计与可复现的验收证据。根目录 README 用于整体定位、已实现功能、演示入口与运行说明；版本文档放在 `docs/iterations/vX.Y/`，旧版本不覆盖。PR 模板会提示更新文档链接。
